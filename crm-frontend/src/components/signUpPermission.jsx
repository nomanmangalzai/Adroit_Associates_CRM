...
const SuperAdminScreen = () => {
  ...
  const approveUserRequest = async (userId) => {
    try {
      ...
      if (response.ok) {
        setResponseMessage(`The user account request has been successfully approved`);
  ...
  const rejectUserRequest = async (userId) => {
    try {
      const response = await fetch(`/api/user/requests/${userId}/reject`, { method: "PUT" });

      if (response.ok) {
        setResponseMessage(`User request with ID ${userId} rejected successfully.`);
