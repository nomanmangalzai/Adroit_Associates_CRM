...
const signUp = async (req,res,next) => {
    ...
    var {firstName, lastName,email, password, confirmPassword, userId} = req.body;
    // console.log(userId)
    // const passwordLength = password.length;

    if(userId){
    var fetchAccountRequestData = await accountRequestCollection.findOne({_id: userId})
...
if (password != confirmPassword) {
  ...
  try {
      ...
      res.status(201).json({ message: "Your account request has been sent to our admin, kindly wait for confirmation" });
    } catch (error) {
      res.status(500).send(error);
    }
