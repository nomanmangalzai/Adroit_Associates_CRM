...
const saveCrmCalendarData = async (req, res, next) => {
  ...
  try {
    ...
    try {
      ...
      res.status(201).json({
        message: "The CRM calendar data has been successfully saved.",
      });
    } catch (error) {
...
const updateCrmCalendarData = async (req, res, next) => {
  ...
  try {
    const existingCrmCalendar = await crmCalendar.findOne({
      organization: organization,
    });
    ...
    if (existingCrmCalendar) {
      ...
      try {
        ...
        res.status(200).json({
          message: "The CRM calendar data has been successfully updated.",
        });
      } catch (error) {
      ...
      res
        .status(404)
        .json({ message: "The CRM calendar data could not be found." });
    }
  } catch (error) {
